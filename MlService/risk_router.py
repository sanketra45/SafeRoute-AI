...
=====================================================
COPIED EXACTLY from your original risk_router.py.
ml_service.py imports compare_routes() and load_graph() from here.
"""
...
# Live conditions are supplied by Spring Boot for every route request.  They are
# deliberately expressed as small, bounded penalties so the distance heuristic
# remains admissible for A* (every edge still costs at least its physical length).
WEATHER_RISK = {
...
def _live_conditions(conditions):
    ...
    # A missing speed must not invent congestion.  TomTom's flow response uses
    # km/h, but only the ratio is relevant here.
...
def _load_blackspot_tree(csv_path):
    ...
    for _, row in df.iterrows():
        try:
            ...
            continue
    # Query a tiny subset of nearby accident records for each road segment
    # instead of comparing every edge with every record (110k x 2k in Nagpur).
    try:
...
def _node_blackspot_score(nlat, nlng, blackspots, radius_m=BLACKSPOT_RADIUS_M):
    ...
    # 1 degree latitude is ~111 km. A slightly generous search radius keeps the
    # subsequent exact haversine calculation correct at Nagpur's longitude.
...
def _nearest_node(G, lat, lng):
    try:
        ...
        # Small test graphs and some GraphML files do not carry OSMnx's CRS
        # metadata; fall back to the dependency-free nearest-node search.
        pass
    ...
    try:
        ...
        def weight_fn(u, v, edge_dict):
            ...
            for key, data in edge_dict.items():
                ...
                # Congestion increases journey time; adverse weather increases
                # the safety penalty. Both values are recalculated per request.
