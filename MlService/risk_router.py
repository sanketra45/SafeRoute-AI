...
def _load_blackspot_tree(csv_path):
    ...
    for _, row in df.iterrows():
        try:
            ...
            continue
    log.info("Loaded %d blackspot records", len(records))
    return records
    ...
    try:
        ...
        def weight_fn(u, v, edge_dict):
            ...
            for key, data in edge_dict.items():
                ...
                if w is None:
                    w = float(data.get("length",50.0)) + float(data.get("risk_score",0.35))*risk_penalty
                min_w = min(min_w, w)
            return min_w
...
def compare_routes(G, origin_lat, origin_lng, dest_lat, dest_lng):
